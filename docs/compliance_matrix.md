...
| 2.3 | Per-surface damage regions with class and metric extent | `cozmo/damage/detect.py` | `damage[]`: surface, class (5), area, width, height, height above floor, outline, confidence, views | **Partial**: built for every tier. On 793 real defect photos (BD3, `bench/results/damage_bd3.json`): 62% flagged, 42% with the right class, 6.5% false alarms on plain walls; cracks about 88%, peeling rarely named right. 0 false regions on our 3 undamaged walks. Metric extent in a real room untested |
...
| B.2 | One furnished room with staged damage, two damage classes | `bench/damage_bd3.py` (public substitute) | Classes checked on 793 real defect photos from BD3 (6 defect classes plus plain walls) | **Not done** as specified: no damaged room was available. The public photos test classes, not metric extent in a room |
