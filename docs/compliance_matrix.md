...
Every requirement in the brief → where it lives → what it is → status. Start at [`SUBMISSION.md`](../SUBMISSION.md) for the reading order and a user guide.
...
| 1.2 | Three input tiers, all mandatory, same output contract | `cozmo/pipeline.py` (`run_lidar`, `run_video`, `run_photos`), `cozmo/ingest/detect.py` | `cozmo run <capture>` detects the tier. All three write the same schema, validated on every run. Input and output formats: `docs/data_formats.md` | **Met** (runs), accuracy per tier below |
| 1.3 | Photos: 2–8 stills per room, per-room folders, stitched whole-property plan, wider intervals | `cozmo/photo/room.py`, `cozmo/stitch/solver.py`, `bench/results/timing.json` | Room box per folder from Depth Anything 3, joined through doors; intervals widened 5.1×. On real iPhone 12 Pro HEIC photos the focal length (the scale input) is read within 1.0% (`bench/iphone_photo_check.py`) | **Partial**: runs end to end, but stitching often leaves several groups (G-PHOTO-STITCH below) |
...
| 2.9 | Rendered plan | `cozmo/export/render.py`, `cozmo/export/report.py` | `plan.svg`, and a one-page visual report (`report.png`, `report.pdf`: plan drawing, room table with ranges, damage, warnings), written next to `result.json` by every run; `cozmo report` redraws it for any result | **Met** |
...
| # | Requirement | File path | Artifact | Status |
|---|---|---|---|---|
| 3.1 | 2 benchmark rooms, LiDAR tier vs one consumer app (named, versioned, export submitted), error by dimension, beat or tie on ≥ 70% | — | — | **Not done**: needs the rooms re-captured with an app such as Polycam or magicplan plus tape truth. We had no device at the end. Script and template ready: `bench/tape_truth.py`, `bench/templates/measurements.csv` |

## Part 4: fix loop

| # | Requirement | File path | Artifact | Status |
|---|---|---|---|---|
| 4.1 | One-page declaration: worst gate with the failing number, root-cause hypothesis and evidence, fix and predicted number | `docs/fix_loop_declaration.md` + `.pdf` (one page); full text in `docs/fix_loop.md` sections 1–4 | Committed before the fix (`3d54e0d`); the one-page copy condenses it with no number changed. G-WALL-VIDEO: 0 of 23 gate walls; predicted 2 | **Met** |
...
| # | Requirement | File path | Artifact | Status |
|---|---|---|---|---|
| 5.1 | Commit as you work | git history | 94 commits, each with its own evidence; predictions committed before results | **Met** |
...
| D.3 | README to running on a fresh capture in < 15 min on a clean machine, one command per capture | `README.md`, `scripts/install.sh`, `scripts/fetch_models.py` | Tested in a fresh venv: install 87 s with warm caches; LiDAR and photo captures ran there (`caefd8c`), and all 121 tests pass at the latest commit. Cold download about 3.3 GB | **Met** on macOS/Apple silicon; Linux untested |
...
| D.6 | Fix loop bundle | `docs/fix_loop_declaration.pdf`, `docs/fix_loop.md`, `bench/results/fix_loop/`, `bench/fix_loop_postmortem.py` | Declaration, runs, diff, post-mortem | **Met** |
...
| # | Requirement | File path | Artifact | Status |
|---|---|---|---|---|
| C.1 | Handheld consumer capture only; any pretrained model or dataset with disclosure; runs without our infrastructure | `README.md` "Models and data used" | DA3 (Apache 2.0), CLIP (MIT), ARKitScenes, HouseLayout3D, BD3 (CC-BY-4.0 re-release), heic.digital iPhone sample photos; runs locally | **Met** |
...
| W.1 | Walk-in test: all three tiers ready to run cold on a new capture | `README.md`, `docs/walk_in.md`, `bench/results/timing.json` | All three tiers run from a fresh install, 23 s to 2.3 min per capture; a cold 37 s video takes 6.3 min, and a 2-minute clip takes about 13 min (key frames capped at 360; a 120-frame cap collapsed the plan). Each run writes `summary.md` (room width × length, ceiling, door widths, with ranges) to read against the laser | **Met** (readiness); accuracy as above |
