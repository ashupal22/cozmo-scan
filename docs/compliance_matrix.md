...
| 2.3 | Per-surface damage regions with class and metric extent | `cozmo/damage/detect.py` | `damage[]`: surface, class (5), area, width, height, height above floor, outline, confidence, views | **Partial**: built for every tier. 0 false regions on our 3 undamaged walks, but painted stains are found in only 16 of 93 images, and it is untested on real damage (`bench/results/damage_sanity.json`) |
...
| B.2 | One furnished room with staged damage, two damage classes | — | — | **Not done**: no damaged room was available to us. The detector is checked on undamaged walks and painted stains only |
...
|---|---|---|---|---|
| G-OPEN | Opening widths ≤ 2 cm on ≥ 85%, missed and phantom openings count | `cozmo/geometry/layout.py` | Widths come from a 5 cm plan grid; no tape truth | **Not measured** |
...
| D.3 | README to running on a fresh capture in < 15 min on a clean machine, one command per capture | `README.md`, `scripts/install.sh`, `scripts/fetch_models.py` | Tested in a fresh venv: install 87 s with warm caches, 112 tests pass, LiDAR and photo captures run. Cold download about 3.3 GB | **Met** on macOS/Apple silicon; Linux untested |
...
| D.8 | Raw benchmark data: sensor logs, ground truth, app exports | Google Drive (`data/README.md`), `docs/datasets.md` | Our raw Stray Scanner captures; public laser truth by script | **Partial**: no tape truth, no app exports |
