...
- **Long videos:** DA3 takes about 3.4 s per key frame on an M4 (378 s for a 37 s clip). `cozmo run` therefore caps a clip at 120 key frames, about 7 minutes cold for any length. On clips over 40 s the frames are spaced further apart, and that accuracy cost is not benchmarked: every committed video result uses all key frames at 3 fps.
