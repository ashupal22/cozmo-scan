...
| **[heic.digital sample photos](https://heic.digital/samples/)**: two iPhone 12 Pro HEIC files | Check that the photo tier reads a real iPhone photo's orientation and focal length, which sets its metric scale | Photo tier, walk-in readiness | The camera's own focal length (4.2 mm lens, 1.4 µm pixels) | Free sample files; downloaded when the check runs, never redistributed | ~5 MB | **Used**: `bench/iphone_photo_check.py` |
