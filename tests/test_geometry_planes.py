...
def _patch(n, height, normal_y, camera_y=1.5, noise=0.01, frame=0):
    xyz = np.column_stack([rng.uniform(-2, 2, n), height + rng.normal(0, noise, n), rng.uniform(-2, 2, n)])
...
def test_floor_is_the_most_supported_up_facing_level():
    pts = _points(_patch(20000, 0.0, 1.0), _patch(5000, 0.75, 1.0))  # floor + table top
...
def test_ceilings_ignore_furniture_and_find_two_heights():
    pts = _points(
        ...
        _patch(4000, 2.27, -1.0),              # lower bathroom ceiling
        _patch(6000, 0.72, -1.0),              # table underside: faces down but far too low
        _patch(3000, 2.10, 1.0),               # top of a tall cabinet: faces up
    )
...
def test_no_ceiling_points_means_no_ceiling():
    pts = _points(_patch(20000, 0.0, 1.0), _patch(6000, 1.20, -1.0))
