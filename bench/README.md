...
These are also the shipped video tier's numbers, with every key frame at 3 fps. `cozmo run` caps key frames at 360 (2 minutes at 3 fps), and the benchmarks pass `max_keyframes=None` so these results still regenerate. A 120-frame cap was measured first, on 1a8384c3f6 (115 s, so 1.05 fps): 1 room instead of 7, footprint -86.0% against -13.0% with all frames, and the footprint interval missed the LiDAR value (`bench/results/video_cap120_1a8384c3f6.json`). DA3 needs neighbouring key frames to overlap, so that cap was dropped. The LiDAR reference moved with the depth correction, so at the latest code the same video plans score -19.5% / -13.0% / -5.4% on footprint (`bench/results/video_vs_lidar_head.json`, `docs/reproduction_check.md`). The fix loop's change made them worse and is switched off, and the re-run with it off reproduces these video plans exactly ([`docs/fix_loop.md`](../docs/fix_loop.md)).
