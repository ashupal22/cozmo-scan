"""Write docs/benchmark_report.md from the result files in bench/results/ and the example runs in examples/, so every
number in the report comes from a committed file.
...
def main():
    ...
    examples = {}
    for f in sorted((ROOT / "examples").glob("*/result.json")):
        d = json.loads(f.read_text())
        examples[f.parent.name] = d

    lines = [
        "# Benchmark report",
        "",
        "Generated by `bench/benchmark_report.py` from `bench/results/` and `examples/`. Methods and discussion are in "
    ...
    for name, d in examples.items():
        lines.append(f"| {name.split('_', 1)[1]} | {d['capture']['tier']} | {len(d['rooms'])} | {d['capture']['runtime_s']} s | "
                     f"{'model outputs cached' if d['capture']['tier'] == 'video' else 'live'}; damage detection adds 20–60 s |")
    cold = ROOT / "bench" / "results" / "timing_cold_video.json"
    if cold.is_file():
        t = json.loads(cold.read_text())
        lines.append(f"| {t['capture']} | video, cold cache | {t['rooms']} | {t['seconds']} s wall | DA3 runs live on every key frame |")
